...
// Prefer NEXT_PUBLIC_APP_URL — it's the same value microsoftRedirectUri() uses
// to build the redirect_uri Microsoft just accepted, so if we got here it's
// already known-good. req.url is only a fallback: behind a reverse proxy that
// doesn't forward the original Host header, req.url can resolve to the
// container's internal bind address (e.g. 0.0.0.0:3000) instead of the public domain.
